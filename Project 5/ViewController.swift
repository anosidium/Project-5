...
class ViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        let recogniser = NSClickGestureRecognizer(target: self, action: #selector(mapClicked))
    ...
    func addPin(at coordinate: CLLocationCoordinate2D) {
        ...
        let guess = Pin(title: "Your guess", coordinate: coordinate, color: NSColor.red)
        ...
        // Calculate how many kilometres they were off, then subtract that from 500
    ...
    @objc func mapClicked(recogniser: NSClickGestureRecognizer) {
        if mapView.annotations.count == 0 {
            addPin(at: mapView.convert(recogniser.location(in: mapView), toCoordinateFrom: mapView))
        } else {
            mapView.removeAnnotations(mapView.annotations)
            nextCity()
        }
        
//        let location = recogniser.location(in: mapView)
...
extension ViewController: MKMapViewDelegate {
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        // 1: Convert the annotation to a pin so we can read its colour
        guard let pin = annotation as? Pin else { return nil }
        
        // 2: Create an identifier string that will be used to share pins
        let identifier = "Guess"
        
        // 3: Attempt to dequeue a pin from the re-use queueu
        ...
        // 6: Customise the pin so that it can show a call out and has a colour
